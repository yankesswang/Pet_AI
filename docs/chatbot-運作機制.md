...
```
已核准段落（白名單） → [LLM 改寫] → 保留度(0.55) → 安全語義 → 數字 → 政策掃描 → 輸出
                                        ↓ 任一關失敗
                                     退回原始段落
```

改寫檢查量的是 **retention（原文還剩多少）**，不是 coverage（改寫有多少能對上原文）。
兩者方向相反，用途也不同：

| 指標 | 分母 | 問的問題 | 用於 |
|---|---|---|---|
| `coverage(主張, 來源)` | 主張 | 這個主張能在來源找到嗎 | 主張驗證 |
| `retention(改寫, 原文)` | 原文 | 原文的內容還在嗎 | 改寫檢查 |

**為什麼改寫不能用 coverage**：內容詞是中文 2-gram，插入一個虛詞
（「是」→「都是」）就會產生原文不存在的邊界 bigram。而「把長句拆短、
加上白話說明」正是 SYSTEM_PROMPT 要求的事，於是 coverage 會系統性懲罰
它自己要求的行為。實測 24 段真實改寫，`coverage>=0.60` 只通過 9 段（38%），
被擋下的全部是忠實且安全的改寫 —— 等於這個功能持續付出 LLM 呼叫成本卻永遠拿不到結果。

「加入了來源沒有的內容」改由三道更直接的確定性檢查負責：

1. **安全語義群**：來源命中「立即／就醫／禁止／危及／必須」任一群時，改寫必須仍命中同一群。
   用群比對，「勿自行使用」→「不要自行使用」不會被誤判；
   但「必須立即就醫」→「也許可以找時間看看」會被擋下。
2. **數字**：改寫不得出現來源沒有的數字（劑量、次數、時間窗）。
3. **政策掃描**：`policy.redact` 攔截劑量、確診、購買等角色違規 ——
   這一關本來就擋得住模型自行加上的劑量與診斷結論。
