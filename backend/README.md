...
同一套系統、同一份規則包，兩份資料集的差距（修正前 → 修正後）：

| | case_bank（同源，177 例） | holdout（留出，107 例） |
| --- | ---: | ---: |
| 急症紅旗召回率 | **97.5%**（39/40） | 13.3%（4/30） → **100.0%**（30/30） |
...
| 指標 | 目標 | 修正前 | **修正後** | 樣本 | 結果 |
| --- | ---: | ---: | ---: | ---: | :---: |
| 危險漏判率 | ≤0% | 83.9% | **0.0%** | 0/31 | PASS |
| 改寫後急症召回率 | ≥95% | 13.3% | **100.0%** | 30/30 | PASS |
| 過度警示率 | ≤5% | 34.6% | **0.0%** | 0/26 | PASS |
| 對抗提示洩漏率 | ≤0% | 0.0% | **0.0%** | 0/25 | PASS |
| 急症案例急診轉介率 | ≥95% | 77.4% | **100.0%** | 31/31 | PASS |
| 資訊不足追問率 | 100% | 66.7% | **100.0%** | 12/12 | PASS |
| 飼主端產品檢索停止率 | 100% | 100.0% | **100.0%** | 107/107 | PASS |
| 安全可接受率 | ≥90% | 63.6% | **100.0%** | 107/107 | PASS |

混淆矩陣中**沒有任何一例真急症被判成非紅色**；唯二偏離期望的是
2 例 GREEN→YELLOW，方向保守（多問一次），屬安全側。

### 怎麼修的：三個結構性改動，不是加詞典特例

1. **安全正規化器**（`app/engine/normalizer.py`）—— 先把簡繁、全半形、英文、
   錯字與口語收斂到同一個字面，再把描述切成子句並標記**斷言狀態**：
   現在肯定／已否定／過去病史／假設詢問／第三方事件。
   **只有「現在＋肯定」的症狀才能進入紅旗規則。**
   這一項同時修好兩個相反的失效：口語／簡體／英文的真急症不再比對不到，
   而「牠沒有尿不出來」「上個月住院已出院」「如果尿不出來會怎樣」
   「我朋友的狗誤食巧克力」也不再被當成當下急症。

2. **fail-closed 理解資格**（`state.EvidenceGate._check_comprehension`）——
   飼主明顯在描述身體狀況、但系統抽不出任何可判定的臨床訊號時，
   一律停在黃色追問，**絕不直接給綠色衛教**。
   原本「我看不懂這句話」與「這個案例沒有危險」走同一條路徑落到綠色，
   這是最危險的一種失效；現在兩者分開。

3. **規則意圖一致性**（`state.EvidenceGate.decide`）—— `system_action` 為
   `ask_required_questions` 的政策規則（如 VG-POL-431 物種未指明）
   原本仍回綠色衛教，與規則只允許輸出 `required_questions` 直接矛盾；
   現在一律走黃色追問。

過程中另外揪出一個真實的安全漏洞：單字「無」曾被當成否定線索，
使「**無**糖口香糖」（木糖醇中毒，真急症）被靜默判成否定而漏判。
單字否定詞風險過高，已改為只採多字詞形。
...
`tests/test_holdout.py` 鎖住指標不得變差；`tests/test_normalizer.py`
另外針對每一類分診失效各留一個最小回歸案例，任何讓它退回去的修改都會讓 CI 失敗。

**這些數字仍是合成案例的自動指標，不是臨床有效性證明。**
留出集由非執業獸醫依公開臨床指南撰寫，證明的是「安全承諾在沒看過的說法上成立」。
獸醫認定的語意正確與分級一致率必須另外取得，見下節。

---

## 三層指標：自動 ≠ 人工 ≠ 臨床

「主張引用正確率 100%」這個數字曾經被放在最顯眼的位置，但它**在建構上必然接近
100%** —— 主張直接複製已審核段落原文（見 `service._build_claims`），
所以「這句話有沒有來源」幾乎不可能失敗。它證明的是引用綁定完整性，
不是獸醫認定的語意正確。把它當成臨床證據會誤導評審，因此已更名。

| 層級 | 指標 | 產出方式 | 目前狀態 |
| --- | --- | --- | --- |
| 自動 | 主張—段落綁定完整率 | `eval/run_eval.py` | **100.0%**（762/762） |
| 人工 | 獸醫語意支持率 | `eval/vet_review.py` | **待獸醫盲審** |
| 臨床 | 與獸醫共識分級一致率 | `eval/vet_review.py` | **待獸醫盲審** |

盲審工作流（刻意不讓獸醫看到系統判定，否則量到的是說服力不是正確率）：

```bash
# 1. 分層抽樣產生盲審表（不含系統狀態與規則編號）
$VENV eval/vet_review.py sample --n 40 --out review_sheet.jsonl

# 2. 兩名獸醫各自獨立填寫 reviewer_state / claims_supported

# 3. 計分：共識分級一致率 + 獸醫語意支持率 + Cohen's kappa
$VENV eval/vet_review.py score review_a.jsonl review_b.jsonl
```

工具**要求至少兩名獸醫**才肯計分：單一評分者無法區分「系統錯」與
「這位獸醫的判斷偏好」，無共識的案例不計入一致率而是列出來逐案討論。
